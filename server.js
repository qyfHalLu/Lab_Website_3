...
app.get('/team_stats', function(req, res){
  ...
  var winc = 'select count(*) as num from football_games where home_score > visitor_score;';
  var lossc = 'select count(*) as num from football_games where visitor_score > home_score;';
  db.task('get-everything', task =>{
  	return task.batch([
...
{
	var choose_players = "SELECT * FROM football_players;";
	db.any(choose_players)
		.then(data => {
...
app.get('/player_info/select_player', function(req, res)
{
	...
	var choose_players = "SELECT id,name FROM football_players;";
	...
	db. task('get-everything', task => {
		return task.batch([
			task.any(choose_players),
			task.any(player_info),
			task.any(football_games)

			])
		.then (data => {
			res.render('pages/playyer_info', {
				title: 'Player Info',
				players : data[0],
				player_info: data[1][0],
				total_played: data[2][0].num
			})
		})
	.catch (error => {
		console.log('error', error);
		rres.redner('pages/player_info', {
			title : 'Player Info',
			players: '',
			player_info: '',
			total_played: ''
